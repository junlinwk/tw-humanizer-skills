...
│   └── 9_teaching/                   學習歷程追蹤
├── presets/                          子情境的詳細規則
│   ├── A_raw_journal.md              → 8 創作 子情境
│   ├── B_casual_journal.md           → 8 創作 子情境
│   ├── C_formal_application.md       → 5 正式 子情境（自傳 / 推薦信）
│   ├── D_report_personal.md          → 5 正式 子情境（學生手寫風報告）
│   ├── E_academic.md                 → 5 正式 子情境（英文學術寫作）
│   └── legacy/                       更早版本（封存）
├── examples/
...
本 skill 不是憑空設計，是從真實人寫樣本逆向歸納的，樣本涵蓋：

- 私人雜記與旅遊日記
- 大學申請的學習歷程自述
- 多份大學課程期初報告（含技術導向與學術導向）
...
原始樣本不收錄於本 repo（見 `.gitignore` 中的 `Reference/` 排除），但其貢獻對 skill 的設計極為關鍵。
